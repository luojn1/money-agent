...
export function UploadPage() {
  ...
  const startAnalysis = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    try {
      const task = await api.createDemoAnalysis();
  ...
  return (
    <PageShell>
      <main className="upload-page">
        ...
        <form className="analysis-form" onSubmit={startAnalysis} noValidate>
          <section className="form-section" aria-labelledby="contract-input-title">
            ...
            >
              ...
              <span>{selectedFile ? "已选择文件，本轮仅模拟分析" : "支持 PDF、JPG、JPEG、PNG"}</span>
            </button>
          ...
          <p className="privacy-note"><ShieldCheck size={18} weight="duotone" />本演示不会上传或保存真实文件，请勿提交敏感个人信息。</p>
        </form>
      </main>
    </PageShell>
  );
