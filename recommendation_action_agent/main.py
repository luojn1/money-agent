...
def run(b_path, c_path, user_profile=None, use_llm=False):
    ...
    # 上游 partial 或链路不一致 -> D partial 并透传警告（协议第 5 节）
    upstream_partial = "partial" in (b_env["status"], c_env["status"])
    status = "partial" if (upstream_partial or pair_problems) else "completed"
    warnings = _carry_warnings(b_env, c_env, extra_warnings) \
        if status == "partial" else _carry_warnings(b_env, c_env)
...
def main():
    ...
    parser.add_argument("--schema", default=None,
                        help="A 的 analysis-protocol-v1.schema.json 路径（可选）")
    ...
    if args.schema and os.path.exists(args.schema):
        schema_problems = schema_check(d_env, args.schema)
