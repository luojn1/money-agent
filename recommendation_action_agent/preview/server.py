...
C_DIR = os.environ.get(
    "C_DIR",
    os.path.normpath(os.path.join(BASE, "..", "..", "C", "risk_case_agent")))
...
def _call_c(b_path, c_path):
    """调用 C 模块命令行做风险识别与案例匹配。

    返回 C 打印到 stdout 的 trace（含 riskScore 0-100 评分，
    评分由 C 的规则引擎按权重扣分计算，说明书 4.2 功能 5）。
    """
    if not os.path.isdir(C_DIR):
        raise PipelineError(f"找不到 C 模块目录：{C_DIR}")
    proc = subprocess.run(
        [sys.executable, "main.py", "--input", b_path, "--output", c_path],
        cwd=C_DIR, capture_output=True, text=True, timeout=180)
