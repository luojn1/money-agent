...
# 注意：不再直接复读 questionToAsk（问题清单里已有原句），这里写"用户具体怎么做"，
# 两个板块各司其职：问题清单=照着问，建议=照着做。
ACTION_TEMPLATE = {
...
# C 的 possibleConsequence 兜底句（全类别同一句），命中时替换为分类别的具体后果。
...
# reason 开头的机器腔："命中规则“X”" -> "合同里存在“X”的情形"
_RULE_PREFIX = "命中规则"


def _short_reason(risk):
    """C 的 reason 很长（含法规摘要），取第一句做 rationale 主体，并去掉机器腔。"""
...
def _merge_duplicate_actions(recs):
    """同类别多条风险会生成相同 action，合并为一条：
    风险关联取并集、优先级取更高档，并在理由末尾注明覆盖了几处条款。"""
    merged, order = {}, []
    for rec in recs:
        key = rec["action"]
        if key in merged:
