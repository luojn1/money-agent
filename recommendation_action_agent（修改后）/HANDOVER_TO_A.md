...
协议 Schema 默认在仓库 `shared/schemas/analysis-protocol-v1.schema.json`
自动发现（也可 `--schema` 或环境变量 `SCHEMA_PATH` 显式指定）。
...
- `partial`：B 或 C 为 partial；warnings 内有 `B_`/`C_` 前缀的透传警告。
- `failed`：B 或 C 为 failed，**或 B/C 不属于同一次分析**（taskId/contractId 不一致、C.inputRunIds 未含 B.runId，errors code 为 `UPSTREAM_LINK_MISMATCH`）；`data = null`，请让任务进入 failed 分支，不要读 data。绝不把两份不同合同的数据拼在一起生成建议。
