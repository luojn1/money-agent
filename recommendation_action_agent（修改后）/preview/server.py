...
# （合并仓库 agents/risk_case 优先，其次本地开发布局 C/risk_case）
_C_CANDIDATES = [
    os.path.join(BASE, "..", "risk_case"),                 # repo/agents/ 下互为兄弟目录
    os.path.join(BASE, "..", "..", "agents", "risk_case"),
    os.path.join(BASE, "..", "..", "C", "risk_case"),      # 本地开发布局
    os.path.join(BASE, "..", "..", "C", "risk_case_agent"),  # 旧本地布局，兼容保留
]
...
_C_OPTIONAL_FLAGS = None


def _c_optional_flags():
    """探测 C 命令行支持的可选参数（不同版本的 C 参数不同）。

    --strict-protocol：要求 C 输出严格协议 JSON（新版 C 默认严格，
    已去掉该参数；旧版 C 需要显式传入）。
    --trace：让 C 把 trace（含 riskScore 评分）打印到 stdout。
    """
    global _C_OPTIONAL_FLAGS
    if _C_OPTIONAL_FLAGS is None:
        try:
            proc = subprocess.run(
            ...
            # 探测失败不阻断链路：按不带可选参数调用 C
            help_text = ""
        _C_OPTIONAL_FLAGS = [flag for flag in ("--strict-protocol", "--trace")
                             if flag in help_text]
    return _C_OPTIONAL_FLAGS
...
def _call_c(b_path, c_path):
    ...
    cmd = [sys.executable, "main.py", "--input", b_path, "--output", c_path]
    cmd += _c_optional_flags()
    try:
        proc = subprocess.run(
            cmd, cwd=C_DIR, capture_output=True, text=True, timeout=180)
