...
正式输出默认已经是严格协议 JSON；如需查看法规、案例、知识库使用统计等扩展信息，请指定 trace 文件：
...
```text
请读取 agents/risk_case/outputs/c-risk-case-output.json，基于其中 riskItems 生成符合 A 协议的 RecommendationActionOutput，recommendations[].relatedRiskIds 必须引用 riskItems[].id。
```
