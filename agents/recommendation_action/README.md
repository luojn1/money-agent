...
C 模块目录按仓库相对路径自动发现（`agents/risk_case`），也可用环境变量覆盖：`export C_DIR=/path/to/agents/risk_case`。
调用 C 时按新版协议传 `--trace` 以从 stdout 读取 riskScore；新版 C 默认严格协议输出，不再传 `--strict-protocol`。
...
当前 16 个测试全部通过（含对 A 官方 JSON Schema 的校验用例）。

## 协议要点

D 输出外层是通用 Agent 信封，`agent` 固定 `recommendation_action`，`agentVersion` 当前 `d-0.2.0`。`data` 按协议**只含四个字段**：
